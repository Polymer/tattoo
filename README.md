...
## GitHub token
 ...
 2. Make it available to tattoo:
    - Put the token in a file called `github-token`
    - Pass it to tattoo every time with `--github-token` CLI option.
    - Put it in your `tattoo_config.json` file with key `"github-token"`
